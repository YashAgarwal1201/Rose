...
<template>
  <div class="p-4 md:p-8 lg:p-10 flex flex-col min-h-full">
    ...
    <div v-if="!summary.isEmpty.value" class="relative mt-6 max-w-2xl shrink-0">
      ...
      <div
        v-else-if="query.trim() && summary.isLoaded.value"
    ...
    <div v-if="summary.isEmpty.value" class="m-auto flex flex-col items-center justify-center pb-20 text-center">
      ...
      <h2 class="text-lg font-semibold text-rose-text">Nothing here yet</h2>
    ...
    <template v-else>
      ...
      <div v-if="summary.topFolders.value.length > 0" class="mt-8">
        <h2 class="text-sm font-semibold text-rose-text-muted uppercase tracking-wide">Folders</h2>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5 mt-3">
          <HomeFolderTile
            v-for="folder in summary.topFolders.value"
            :key="folder.id"
            :folder="folder"
            :route-name="folderRouteName(folder.type)"
          />
          <div
            v-if="summary.overflowFolderCount.value > 0"
      ...
      <div v-if="summary.recentItems.value.length > 0" class="mt-8">
        <h2 class="text-sm font-semibold text-rose-text-muted uppercase tracking-wide">Recent</h2>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5 mt-3">
          <HomeFileCard
            v-for="item in summary.recentItems.value"
