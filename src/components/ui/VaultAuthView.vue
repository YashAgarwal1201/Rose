<template>
  <div class="flex h-full w-full flex-col overflow-y-auto p-4 md:p-8 text-center text-slate-800 dark:text-slate-200">
    <div class="m-auto flex w-full max-w-md flex-col items-center justify-center py-2 md:py-4 shrink-0">
        ...
        <form @submit.prevent="handleSetup" class="space-y-3 md:space-y-4">
          ...
          <div>
            ...
            <input v-model="pin" id="new-pin" name="new-password" autocomplete="new-password" type="password" required
              class="w-full rounded-md border border-slate-300 px-3 py-1.5 md:px-4 md:py-2 dark:border-slate-700 dark:bg-slate-900 focus:border-rose-500 focus:ring-1 focus:ring-rose-500"
      ...
      <div v-else-if="setupPhase === 'recovery'" class="max-w-md w-full">
        <div
          class="p-4 mb-6 rounded-md bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 text-left">
          ...
          <div
            class="p-3 bg-white dark:bg-black rounded border border-yellow-300 dark:border-yellow-600 font-mono text-center tracking-widest text-lg select-all">
      ...
      <div v-else class="max-w-sm w-full">
        <p class="mb-3 md:mb-6 text-sm md:text-base text-slate-500 dark:text-slate-400">
        ...
        <form @submit.prevent="handleUnlock" class="space-y-4">
          ...
          <input v-model="pin" id="pin" name="password" autocomplete="current-password" type="password" required
            autofocus
            class="w-full rounded-md border border-slate-300 px-4 py-2 md:py-3 text-center text-xl tracking-[0.5em] dark:border-slate-700 dark:bg-slate-900 focus:border-rose-500 focus:ring-1 focus:ring-rose-500"
            placeholder="••••" />

          <div v-if="errorMsg" class="text-sm text-red-500 font-medium">{{ errorMsg }}</div>

          <button type="submit"
            class="w-full rounded-md bg-rose-600 px-4 py-1.5 md:py-2 text-sm md:text-base font-medium text-white hover:bg-rose-700 transition-colors">
            Unlock
          </button>
        </form>

        <button @click="showRecoveryMode = !showRecoveryMode"
          class="mt-3 md:mt-6 text-sm text-slate-500 hover:text-rose-500 transition-colors">
          Forgot PIN?
        </button>
