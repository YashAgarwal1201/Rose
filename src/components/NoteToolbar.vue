...
const startPlacement = computed<PopoverPlacement>(() => {
  if (props.position === "left") return "right-start";
  if (props.position === "right") return "left-start";
  if (props.position === "bottom") return "top-start";
  return "bottom-start";
});
...
<template>
  ...
  >
    <button
      ...
      :class="tool === 'select' ? activeBtn : idleBtn"
      title="Select"
      @click="selectTool('select')"
    >
      <MousePointer2Icon class="w-[18px] h-[18px]" />
    </button>
    <button
      ref="penTriggerRef"
      type="button"
      class="p-2 rounded-md transition-colors"
      :class="tool === 'pen' ? activeBtn : idleBtn"
      title="Pen"
      @click="togglePenPopover"
    >
      <PenIcon class="w-[18px] h-[18px]" />
    </button>
    <button
      type="button"
      class="p-2 rounded-md transition-colors"
      :class="tool === 'eraser' ? activeBtn : idleBtn"
      title="Eraser"
      @click="selectTool('eraser')"
    >
      <EraserIcon class="w-[18px] h-[18px]" />
    </button>
    <button
      ref="colorTriggerRef"
      type="button"
    ...
    >
      <span
        class="block w-[18px] h-[18px] rounded-full border border-rose-border"
    ...
    <button
      ...
      type="button"
      class="p-2 rounded-md transition-colors"
      :class="tool === 'shape' ? activeBtn : idleBtn"
      title="Shapes"
      @click="toggleShapePopover"
    >
      <SquareIcon class="w-[18px] h-[18px]" />
    </button>
    <button
      type="button"
      class="p-2 rounded-md transition-colors"
      :class="tool === 'text' ? activeBtn : idleBtn"
      title="Text box"
      @click="selectTool('text')"
    >
      <TypeIcon class="w-[18px] h-[18px]" />
    </button>
    <button
      type="button"
      class="p-2 rounded-md transition-colors"
    ...
    <button
      ...
      @click="toggleBackgroundPopover"
    >
      <PaletteIcon class="w-[18px] h-[18px]" />
    </button>
    ...
    <button
      type="button"
      class="p-2 rounded-md transition-colors disabled:opacity-30 disabled:pointer-events-none"
      :class="idleBtn"
      :disabled="!canUndo"
      title="Undo"
      @click="emit('undo')"
    >
      <Undo2Icon class="w-[18px] h-[18px]" />
    </button>
    <button
      type="button"
      class="p-2 rounded-md transition-colors disabled:opacity-30 disabled:pointer-events-none"
      ...
      @click="emit('redo')"
    >
      <Redo2Icon class="w-[18px] h-[18px]" />
    </button>

    <div
      v-if="isPenPopoverOpen"
      ref="penPopoverRef"
    ...
    >
      <button
        v-for="preset in ['pencil', 'pen', 'marker'] as PenTool[]"
        :key="preset"
        type="button"
        class="px-2.5 py-1.5 rounded-md text-sm text-left capitalize transition-colors"
        :class="
          penTool === preset
    ...
    >
      ...
      >
        <span
          class="w-4 h-4 rounded-full border border-rose-border"
